    ...
    <>
      ...
      >
        ...
        <motion.div
          initial={{ y: -500 }}
          animate={{ y: 0 }}
          transition={{ duration: 1, delay: i + i, type: "tween" }}
          className="colortheme  z-50 [&>div]:cursor-pointer blocks flex flex-row fixed [&>div]:h-8 [&>div]:relative top-0 left-50% "
        >
          <div
            id="red"
            onClick={() => setTheam("red")}
            className="block__item bg-gradient-to-tr from-red-500 to-red-700"
          >
            {theme === "red" ? (
              <div className="red text-red-700 text-5xl dot  ">.</div>
            ) : (
              <div></div>
            )}
          </div>
          <div
            id="blue"
            onClick={() => setTheam("blue")}
            className="block__item bg-gradient-to-tr from-blue-500 to-blue-700 "
          >
            {theme === "blue" ? (
              <div className="red text-blue-700 text-5xl  dot ">.</div>
            ) : (
              <div></div>
            )}
          </div>
          <div
            id="green"
            onClick={() => setTheam("green")}
            className="block__item bg-gradient-to-tr from-green-500 to-green-700"
          >
            {theme === "green" ? (
              <div className="red text-green-700 text-5xl dot ">.</div>
            ) : (
              <div></div>
            )}
          </div>
          <div
            id="purple"
            onClick={() => setTheam("purple")}
            className="block__item bg-gradient-to-tr from-purple-500 to-purple-700 "
          >
            {theme === "purple" ? (
              <div className="red text-purple-700 text-5xl dot ">.</div>
            ) : (
              <div></div>
            )}
          </div>
          <div
            id="pink"
            onClick={() => setTheam("pink")}
            className="block__item bg-gradient-to-tr from-pink-500 to-pink-700"
          >
            {theme === "pink" ? (
              <div className="red text-pink-700 text-5xl dot ">.</div>
            ) : (
              <div></div>
            )}
          </div>
        ...
        <div className="text relative z-40  my-auto font-[Inter] flex items-center justify-center flex-col">
          <div className="head w-full sm:w-2/3 lg:w-full flex flex-wrap space-y-6 min-[310px]:space-y-0 lg:space-x-6 lg:flex-nowrap flex-row justify-start sm:justify-center items-center ml-8 sm:-ml-8 md:ml-0 h-40 sm:h-52 font-[700] md:text-8xl">
            ...
            <motion.span
              className="text-6xl hidden  sm:text-7xl ml-2 items-center min-[310px]:flex"
              initial={{ opacity: 0 }}
              animate={{
          ...
          <motion.div
            ...
            transition={{ duration: 1, delay: i + 2.8 }}
            animate={{ opacity: 1, y: 0 }}
            style={{ color: "rgba(255, 255, 255, 0.7)" }}
            className="desc w-full flex justify-start sm:justify-center lg:-mt-7 text-lg  min-[310px]:text-2xl md:text-4xl items-center ml-10 md:ml-0 mt-2 sm:mt-2 "
          >
            <TypeAnimation
              sequence={[
        ...
        <div className="scroll relative z-40 items-center flex flex-row ml-14 ">
          ...
          >
            <path
              d="M7.29289 73.7071C7.68342 74.0976 8.31658 74.0976 8.70711 73.7071L15.0711 67.3431C15.4616 66.9526 15.4616 66.3195 15.0711 65.9289C14.6805 65.5384 14.0474 65.5384 13.6569 65.9289L8 71.5858L2.34315 65.9289C1.95262 65.5384 1.31946 65.5384 0.928932 65.9289C0.538408 66.3195 0.538408 66.9526 0.928932 67.3431L7.29289 73.7071ZM7 0L7 73H9L9 0L7 0Z"
              fill="white"
              fill-opacity="0.47"
            />
          </motion.svg>
        ...
        <motion.div
          ...
          className="icons z-40 grid sm:grid-cols-1 gap-5 sm:gap-3 grid-cols-2 bottom-5 left-6 sm:bottom-14 sm:left-7 absolute"
        >
